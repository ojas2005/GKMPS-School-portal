...
namespace SchoolERP.Shared.Hosting;

/// <summary>
/// Startup plumbing every service and the gateway share: real client IPs behind the proxy
/// chain, rate-limit partitioning, Swagger gating, and resilient startup migrations.
/// </summary>
public static class SharedHosting
{
    /// <summary>
    /// Traffic reaches a service as browser -> Caddy -> gateway -> service, so
    /// Connection.RemoteIpAddress is always the previous hop. Honour X-Forwarded-For/Proto
    /// from that one hop (ForwardLimit = 1) so rate limiting and audit logs see the real
    /// client. Safe because no service port -- gateway included -- is published beyond
    /// loopback (see docker-compose.yml); only Caddy and the gateway can reach them.
    /// </summary>
